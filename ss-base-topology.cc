...
uint32_t BaseTopology::FindChunkHost(uint32_t chunk_id, uint32_t rack_id)
{
...
uint32_t hashOne(uint32_t a){ return (19*(a+1))%16; }
uint32_t hashTwo(uint32_t a){ return (43*(a+1))%16; }
uint32_t hashThree(uint32_t a){ return (17*(a+1))%16; }
uint32_t hashfour(uint32_t a){ return (23*(a+1))%16; }
uint32_t hashfive(uint32_t a){ return (91*(a+1))%16; }
...
//Parameters
	...
	typedef uint32_t (*IntFunctionWithOneParameter) (uint32_t a);
		...
		for(uint32_t j=0;j<nodes_in_pod;j++)
		{
			...
			//	NS_LOG_UNCOND("--------------------------"<<chunk_num<<"bandwidth "<<BaseTopology::q[i].nodes[j].data[q].intensity_sum);
				...
				//{
					...
					BaseTopology::chnkCopy[chunk_num].primary_location=j+i*Ipv4GlobalRouting::FatTree_k;// changed by madhurima on May 10th
					//copy++;
				//}
...
//we are checking if a node is beyond the high range and below the emergency range
	    if(cuttoffnode_high<max_node_u /* && cuttoffnode_emer>max_node_u*/ && beyond_cutoff>0.0 )
	    {
		   ...
		   if(max_chunk_no!=9999)
		   {
			   ...
			   //hash the chunk number with some function
			   ...
			   //int newlocation=(max_chunk_no*5+2)%15;
			   for(int z = 0; z < 3; ++z)
			   {
				   int newlocation=functions[z](max_chunk_no);
				   uint32_t node_num=newlocation%Ipv4GlobalRouting::FatTree_k;
				   uint32_t pod_num=newlocation/Ipv4GlobalRouting::FatTree_k;
				   if(BaseTopology::q[pod_num].Pod_utilization_out+estimated_utlization<cuttoffpod)
				   {
					   //check if the copy already exists in that node
						for(uint32_t q=0;q<BaseTopology::q[pod_num].nodes[node_num].total_chunks;q++)
						{
						 if(BaseTopology::q[pod_num].nodes[node_num].data[q].chunk_number==max_chunk_no)
							 {
								 flag=1;
								 break;
							 }
						}
					   //next check for if the node meet the utilization threshold
					   if(BaseTopology::q[pod_num].nodes[node_num].utilization_out+estimated_utlization</*(cuttoffnode_high-(cuttoffnode_high*.3))*/ cuttoffnode_real  && !flag /*&& BaseTopology::q[i].nodes[j].utilization>0 && target_pod>1*/) //unmark
					   {

						   diff_from_node_cutoff=(int(Count)*int(SSD_PER_RACK))-BaseTopology::q[pod_num].nodes[node_num].utilization_out;//+estimated_utlization); //unmark
			   ...
			   if(!found)
				{
					NS_LOG_UNCOND("NF NF NF NF NF NF NF NF");
					target_node=999;
					target_pod=999;
				}
//			if(energy){
...
//							 //register benefit - lookin0g for a node which has utilization less than the real cutoff but still very near to reall cutoff
//							 if(max_diff_from_node_cutoff>diff_from_node_cutoff)
//							 {
//								 max_diff_from_node_cutoff=diff_from_node_cutoff;
//								 target_node=j;
			   ...
			   for(uint32_t i = 0; i<BaseTopology::q[max_pod].nodes[max_node_no].total_chunks; i++)
					 {
						 ...
						 float write_ratio=float(BaseTopology::chnkCopy[l].writeCount)/deno;
						 //write_ratio=std::ceil(write_ratio * 100) / 100;
						 uint32_t chunkonNode=0;
						 if (simulationRunProperties::enableCopy!=1)
						 {
							 NS_LOG_UNCOND(BaseTopology::q[max_pod].nodes[max_node_no].data[i].chunk_number<<"    "<< (max_pod*Ipv4GlobalRouting::FatTree_k)+max_node_no);
							 uint32_t chunk_location = FindChunkHost(BaseTopology::q[max_pod].nodes[max_node_no].data[i].chunk_number, (max_pod*Ipv4GlobalRouting::FatTree_k)+max_node_no);

							 for (uint32_t f=0;f<simulationRunProperties::total_chunk;f++)
							 {
									if(BaseTopology::chunk_copy_node_tracker[f][chunk_location] == true)
										{
											chunkonNode++;
											if(chunkonNode>1)
												break;
						...
						if(/* BaseTopology::q[max_pod].nodes[max_node_no].data[i].highCopyCount<Ipv4GlobalRouting::FatTree_k
							 ...
							 && ((chunkonNode>1 && simulationRunProperties::enableCopy!=1)||(simulationRunProperties::enableCopy==1)))
		   ...
		   if(max_chunk_no!=9999)
		   {
			   ...
			   max_diff_from_node_cutoff=0.0;
				...
				for(int z = 0; z < 3; ++z)
				{
					...
					uint32_t node_num=newlocation%Ipv4GlobalRouting::FatTree_k;
					uint32_t pod_num=newlocation/Ipv4GlobalRouting::FatTree_k;
					if(BaseTopology::q[pod_num].Pod_utilization+estimated_utlization<cuttoffpod)
					{
						//check if the copy already exists in that node
						for(uint32_t q=0;q<BaseTopology::q[pod_num].nodes[node_num].total_chunks;q++)
						{
					...
					//next check for if the node meet the utilization threshold
						if(BaseTopology::q[pod_num].nodes[node_num].utilization+estimated_utlization</*(cuttoffnode_high-(cuttoffnode_high*.3))*/ cuttoffnode_real  && !flag /*&&  target_pod>1 && BaseTopology::q[i].nodes[j].utilization>0*/) //unmark
						{
							if(BaseTopology::q[pod_num].nodes[node_num].utilization+estimated_utlization</*(cuttoffnode_high-(cuttoffnode_high*.3))*/ cuttoffnode_real  && !flag /*&& BaseTopology::q[i].nodes[j].utilization>0*/) //unmark
							{
								diff_from_node_cutoff=(int(Count)*int(SSD_PER_RACK))-BaseTopology::q[pod_num].nodes[node_num].utilization;//+estimated_utlization); //unmark
								if(max_diff_from_node_cutoff<diff_from_node_cutoff)
								{
									max_diff_from_node_cutoff=diff_from_node_cutoff;
									target_node=node_num;
